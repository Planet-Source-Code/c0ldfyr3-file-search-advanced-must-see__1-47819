...
Private Sub cmdSearch_Click()
    ...
    Select Case cmdSearch.Caption 'Easy method of Starting and Stopping, detect the button caption =)
        Case Is = "Search Now" 'Start Searching
            ...
            sPaths = txtFolder.Text
            If Left$(sPaths, 19) = "Local Hard Drives (" Then
                sPaths = Mid$(txtFolder.Text, 20, Len(txtFolder.Text) - 20)
            Else
                sPaths = txtFolder.Text
            End If
            If InStr(1, sPaths, ";") > 0 Then
                sSearchPaths = Split(sPaths, ";")
            Else
                ReDim sSearchPaths(0)
                sSearchPaths(0) = sPaths
            End If
            For X = 0 To UBound(sSearchPaths)
                If Len(Dir(sSearchPaths(X), vbDirectory)) = 0 Then
                    Call MsgBox("You must enter a valid search path !" & vbCrLf & vbCrLf & "Source: " & sSearchPaths(X), vbExclamation, "Error")
                    Exit Sub
                End If
            Next
            For X = 0 To UBound(sSearchPaths)
                If bContinue = False Then Exit For
                If X = UBound(sSearchPaths) Then bContinue = False
                sSearchPaths(X) = FixPath(sSearchPaths(X))
                ...
                With cFind
                    ...
                    .FileSpec = IIf(Len(txtSearch.Text) = 0, "*.*", IIf(InStr(1, txtSearch.Text, "*") = 0, "*" & txtSearch.Text & "*", txtSearch.Text)) 'What we need to search for.
                    cmdSearch.Caption = "Stop"
                    'If .Searching = False Then .Searching = True
...
Private Sub FrameXPMenu_Click(Index As Integer)
Debug.Print FrameXPMenu(Index).Height
End Sub
