...
Public Sub Main()
    ...
    If Len(sTmp) = 0 Then tVar = vbChecked
    FrmMain.chkSubFolders.Value = IIf(sTmp = "1", vbChecked, tVar)
    tVar = vbUnchecked
    sTmp = GetSetting("Search Hidden Folders")
    If Len(sTmp) = 0 Then tVar = vbChecked
    FrmMain.chkHidden.Value = IIf(sTmp = "1", vbChecked, tVar)
    tVar = vbUnchecked
    sTmp = GetSetting("Search System Folders")
    If Len(sTmp) = 0 Then tVar = vbChecked
    FrmMain.chkSystem.Value = IIf(sTmp = "1", vbChecked, tVar)
    tVar = vbUnchecked
    sTmp = GetSetting("Search Zip Folders")
    If Len(sTmp) = 0 Then tVar = vbChecked
    FrmMain.chkZips.Value = IIf(sTmp = "1", vbChecked, tVar)
    tVar = vbUnchecked
    If Len(FrmMain.txtFolder.Text) = 0 Then
        sArr = GetDriveList
        X = 0
        Do
            If Not GetDriveType(sArr(X)) = enumDriveTypes.DRIVE_FIXED Then
                Call RemoveFromStringArray(sArr, X)
            Else
                sArr(X) = FixPath(sArr(X), False)
                X = X + 1
            End If
        Loop While X <= J_UBound(sArr)
        FrmMain.txtFolder.Text = "Local Hard Drives (" & Join(sArr, ";") & ")"
    End If
